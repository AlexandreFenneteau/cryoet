...
class PreprocData(ABC):
    ...
    def __init__(self, split: str = "train", overwrite: bool = False):
        ...
        self.preproc_folder = os.path.join(
            CONF.DATA_DIR, "preproc", "pytorch", self.split)
        ...
        for experiment_name in get_experiment_names(self.copick_root):
            ...
            if (len(glob.glob(os.path.join(self.preproc_folder, f"{experiment_name}_res-*_img.pt"))) < 3) or self.overwrite:
                if self.split == "train":
                    ...
                    label_dicts = get_run_labels(
                        self.copick_root, experiment_name)
                ...
                for zarr_tensor, res_dict in get_zarr_tensors(self.copick_root, experiment_name):
                    ...
                    if self.split == "train":
                        msk_tensor_dict = self._generate_label_mask(
                            zarr_tensor.shape, res_dict[res_name], label_dicts)
                        all_msk_tensor = torch.zeros(
                            zarr_tensor.shape, dtype=int)
                        for particle_type in msk_tensor_dict:
                            msk_tensor = msk_tensor_dict[particle_type]
                            all_msk_tensor[msk_tensor > 0] = msk_tensor[msk_tensor >
                                                                        0] * self.particle_int_label[particle_type]
                            msk_save_path = os.path.join(
                                self.preproc_folder, f"{experiment_name}_res-{res_name}_msk-{particle_type}.pt")
                            torch.save(msk_tensor, msk_save_path)
                            print(f"{msk_save_path} ... written!")
                        all_msk_save_path = os.path.join(
                            self.preproc_folder, f"{experiment_name}_res-{res_name}_msk-all.pt")
                        torch.save(all_msk_tensor, all_msk_save_path)
                        print(f"{all_msk_save_path} ... written!")

                    tensor_save_path = os.path.join(
                        self.preproc_folder, f"{experiment_name}_res-{res_name}_img.pt")
                    torch.save(zarr_tensor, tensor_save_path)
                    print(f"{tensor_save_path} ... written!")
                exp_res_path = os.path.join(
                    self.preproc_folder, f"{experiment_name}_res-detail.json")
...
class ExportYOLOFormat(ABC):
    ...
    def __init__(self, slicing: str,
                 ...
                 overwrite: bool = False, slicing_radius_ratio: float = 0.9):
        assert slicing in [
            'x', 'y', 'z'], f"{slicing = } should be only 'x', 'y' or 'z'"
        super().__init__()
        self.preproc_pytorch_folder = os.path.join(
            CONF.DATA_DIR, "preproc", "pytorch")
        self.preproc_yolo_folder = os.path.join(
            CONF.DATA_DIR, "preproc", "yolo")
        ...
        for torch_split in ["train", "test"]:
            ...
            for yolo_split in yolo_splits:
                yolo_img_path = os.path.join(
                    self.preproc_yolo_folder, self.slicing_name, "images", yolo_split)
                ...
                for exp_name in self.split_dict[yolo_split]:
                    pt_path = os.path.join(
                        self.preproc_pytorch_folder, torch_split, f"{exp_name}_res-0_img.pt")
                    ...
                    for i in range(n_slices):
                        ...
                        img_path = os.path.join(
                            yolo_img_path, f"{exp_name}_{i:03}.png")
                    ...
                    if yolo_split != "test":
                        ...
                        label_csv_path = os.path.join(
                            self.preproc_pytorch_folder, torch_split, f"{exp_name}_label_coords.csv")
                        res_detail_path = os.path.join(
                            self.preproc_pytorch_folder, torch_split, f"{exp_name}_res-detail.json")

                        yolo_label_path = os.path.join(
                            self.preproc_yolo_folder, self.slicing_name, "labels", yolo_split)
                        ...
                        with open(label_csv_path, 'r') as csv_file:
                            ...
                            for particle_dict in reader:
                                ...
                                r_c = {
                                    key: val / img_shape[self.img_axes_maps[key]] for key, val in v_c.items()}

                                particle_radius_vox = (
                                    float(particle_dict["radius"]) * 3.) / (res['x'] + res['y'] + res['z'])
                                # w, h = label['radius'] / widht and height of image
                                w = particle_radius_vox / \
                                    img_shape[self.width_ax_idx]
                                h = particle_radius_vox / \
                                    img_shape[self.height_ax_idx]
                                ...
                                last_slice = min(center_slice_idx + int(
                                    particle_radius_vox * self.slicing_radius_ratio), img_shape[self.slicing_idx])
                                for i in range(first_slice, last_slice):
                                    ...
                                    label_txt_path = os.path.join(
                                        yolo_label_path, f"{exp_name}_{i:03}.txt")
        ...
        yolo_yml_path = os.path.join(
            self.preproc_yolo_folder, f"{self.slicing_name}_data.yaml")
        yolo_yml_dict = {
            "path": "../cryoet/data/preproc/yolo",
            "train": '/'.join([self.slicing_name, "images", "train"]),
            "val": '/'.join([self.slicing_name, "images", "val"]),
            "test": '/'.join([self.slicing_name, "images", "test"]),
            "names": {
                    0: "apo-ferritin",
                    1: "beta-amylase",
...
if __name__ == "__main__":
    split_dict = {"train": ["TS_5_4", "TS_6_4", "TS_69_2", "TS_86_3", "TS_99_9"],
                  "val": ["TS_6_6", "TS_73_6"],
                  "test": []}
    # train_preproc = PreprocData("train")
    # train_preproc = PreprocData("test")
    # test_preproc = PreprocData("test")
    toto = ExportYOLOFormat('y', overwrite=False,
                            slicing_radius_ratio=0.8,
                            split_dict=split_dict)

    toto = ExportYOLOFormat('x', overwrite=False,
                            slicing_radius_ratio=0.8,
                            split_dict=split_dict)
	

    toto = ExportYOLOFormat('z', overwrite=False,
                            slicing_radius_ratio=0.8,
                            split_dict=split_dict)
